...
- `tests/api/benchmark-test.js` reads a CSV (the Make target supplies `tests/api/cases/address-tagged.csv`), makes requests sequentially using Node's `http` client, and writes correctness results. That current private/ignored accuracy input is a limited dataset; separately, the checked-in historical result files contain 106 records each. The result count reflects the historical accuracy run, not a production-scale or performance dataset. This runner is an accuracy corpus tool, not a load generator.
- `tests/api/cases/address-tagged.csv` and `tests/api/cases/address.csv` are ignored private accuracy inputs. The tagged file had 61 local rows and the full file 109 at audit time; these local counts may vary and are not performance workload counts. Do not publish their raw rows or copy them into public artifacts. The checked-in 106-result accuracy set is historical and does not imply that the current ignored inputs or performance workload contain 106 requests.
...
- `tests/api/cases/address-tagged.csv` has 61 local rows at audit time and is ignored by Git. It carries ground truth/tagging and may contain private inputs. It is relevant to the accuracy benchmark only; it is not the performance workload for this task.
...
- The committed accuracy JSON contains 106 raw addresses/quality outputs per file. Those 106 records are historical accuracy results from a non-committed dataset, not a required performance sample.
- The current k6 load fixture has five hand-selected strings. For this performance task, those five addresses are the intentionally scoped performance workload. It is deterministic and useful for repeatable measurement, but too small and homogeneous to support broad traffic-representativeness claims.
