-- Postgres counterpart of db/location.sql. Same tables, columns and ids; run
-- alongside location.sql by the seeder (--init). Requires the pg_trgm
-- extension for the similarity index on lowercase_normalized.
--
-- Timestamps stay TEXT (matching SQLite); Postgres defaults use now()::text.
-- ids use GENERATED BY DEFAULT AS IDENTITY so the seeder can insert the same
-- explicit ids as SQLite, keeping both stores in lockstep.
...
INSERT INTO location_levels (id, code, name) VALUES
    (1, 1, 'country'),
    (2, 2, 'province'),
    (3, 3, 'city'),
    (4, 4, 'district'),
...
-- Trigram index for similarity / fuzzy matching on lowercase_normalized
-- (supports % , ILIKE '%...%' and similarity()).
CREATE INDEX IF NOT EXISTS location_codes_normalized_trgm_idx
