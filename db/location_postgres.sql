-- Postgres mirror of db/location.sql. The seeder creates these empty tables,
-- copies the complete SQLite snapshot with COPY, then applies the indexes in
-- db/location_postgres_indexes.sql.
--
-- Timestamps stay TEXT (matching SQLite); Postgres defaults use now()::text.
-- ids use GENERATED BY DEFAULT AS IDENTITY so COPY can preserve SQLite ids.
